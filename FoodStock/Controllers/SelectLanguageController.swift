...
class SelectLanguageController: UIViewController, NVActivityIndicatorViewable {
    ...
    func DownloadAppInfo() {
        ...
        self.firebasedownload.DownloadAppInfo { (appInfo, error) in
            ...
            if let error = error {
                ...
                utils.sharedInstance.showAlert(viewController: self, "Loading Error", message: error.localizedDescription)
                return
            }
            
            if let appInfo = appInfo {
                self.DismissLoadingView()
                if let lan = appInfo[Constants.language] {
                    utils.sharedInstance.ChangeLocalizationwithNativeLanguage(lan: lan)
                    self.ConfigurationLanguage(lan: lan)
                }
            }
        }
    }
    ...
    func UploadAppInfo() {
        ...
        let inArray: NSDictionary = [Constants.language: language, Constants.email: "Foodstock2020@gmail.com"]
        
        self.firebaseupload.UpdateAppInfo(with: inArray) { (error) in
            
            if let error = error {
                self.DismissLoadingView()
                utils.sharedInstance.showAlert(viewController: self, "Server Error", message: error.localizedDescription)
            }else {
                self.DismissLoadingView()
                ...
                defaults.set(lan, forKey: Constants.language)
    ...
    func ShowLoadingView() {
        ...
        startAnimating(size, message: "Loading", type: NVActivityIndicatorType.ballSpinFadeLoader, fadeInAnimation: nil)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "main" {
            _ = segue.destination as! SideBarController
        }
    }
